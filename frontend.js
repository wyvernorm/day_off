...
<style>
:root {
  --bg: #f8fafc; --sf: #ffffff; --bd: #e2e8f0; --tx: #1e293b; --ts: #64748b;
  --pr: #4361ee; --pb: #eaefff; --dg: #ef476f; --db: #fff0f3;
  --su: #06d6a0; --sb: #e6faf4; --wn: #ffd166; --wb: #fff8e6;
  --rd: 16px; --sh: 0 2px 12px rgba(0,0,0,.08); --sl: 0 12px 40px rgba(0,0,0,.12);
  --accent: #7209b7;
}
* { box-sizing: border-box; margin: 0; padding: 0; }
body { font-family: 'Noto Sans Thai', -apple-system, sans-serif; background: var(--bg); color: var(--tx); font-size: 15px; line-height: 1.6; -webkit-font-smoothing: antialiased; }
button { font-family: inherit; cursor: pointer; }
::selection { background: var(--pb); color: var(--pr); }
.ctn { max-width: 1440px; margin: 0 auto; padding: 24px; }
.hdr { display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px; flex-wrap: wrap; gap: 16px; padding: 24px; background: var(--sf); border-radius: 20px; box-shadow: var(--sh); }
.hdr h1 { font-size: clamp(24px, 5vw, 32px); font-weight: 800; letter-spacing: -0.5px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.hdr p { font-size: 14px; color: var(--ts); margin-top: 4px; }
.ub { display: flex; align-items: center; gap: 12px; background: var(--sf); padding: 8px 16px; border-radius: 50px; border: 1px solid var(--bd); box-shadow: var(--sh); transition: all 0.3s; }
.ub:hover { box-shadow: 0 4px 20px rgba(0,0,0,.1); transform: translateY(-2px); }
.ua { width: 40px; height: 40px; border-radius: 50%; object-fit: cover; border: 2px solid var(--bd); transition: transform 0.3s; }
.ua:hover { transform: scale(1.1) rotate(5deg); }
.uae { font-size: 32px; line-height: 40px; }
.un { font-weight: 700; font-size: 14px; }
.ur { font-size: 11px; color: var(--pr); background: var(--pb); padding: 3px 10px; border-radius: 12px; font-weight: 600; }
.ubtn { border: none; background: #f1f5f9; padding: 8px 14px; border-radius: 10px; font-size: 13px; font-weight: 600; color: var(--ts); transition: all .2s; }
.ubtn:hover { background: #e2e8f0; transform: translateY(-1px); }
.tabs { display: flex; gap: 6px; background: var(--sf); padding: 6px; border-radius: 16px; border: 1px solid var(--bd); flex-wrap: wrap; box-shadow: var(--sh); overflow-x: auto; scrollbar-width: none; }
.tabs::-webkit-scrollbar { display: none; }
.tab { padding: 12px 20px; border: none; border-radius: 12px; font-size: 14px; font-weight: 700; background: transparent; color: var(--ts); transition: all .3s; white-space: nowrap; position: relative; min-height: 44px; }
.tab::before { content: ''; position: absolute; inset: 0; background: var(--pb); opacity: 0; border-radius: 12px; transition: opacity .3s; z-index: -1; }
.tab:hover::before { opacity: 1; }
.tab:hover { color: var(--pr); transform: translateY(-2px); }
.tab.on { background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; box-shadow: 0 4px 16px rgba(67,97,238,.3); transform: translateY(-2px); }
.mnv { display: flex; align-items: center; gap: 12px; background: var(--sf); padding: 16px 20px; border-radius: var(--rd); border: 1px solid var(--bd); margin-bottom: 20px; flex-wrap: wrap; box-shadow: var(--sh); }
.mnv h2 { font-size: 22px; font-weight: 800; min-width: 200px; text-align: center; letter-spacing: -0.3px; }
.nb { border: none; background: var(--pb); width: 40px; height: 40px; border-radius: 12px; font-size: 18px; font-weight: 700; color: var(--pr); display: flex; align-items: center; justify-content: center; transition: all .3s; min-height: 44px; }
.nb:hover { background: var(--pr); color: #fff; transform: translateY(-2px); box-shadow: 0 4px 12px rgba(67,97,238,.3); }
.nb:disabled { opacity: .3; cursor: not-allowed; transform: none; }
.tb { border: 2px solid var(--pr); background: var(--pb); padding: 10px 20px; border-radius: 12px; font-size: 13px; font-weight: 700; color: var(--pr); transition: all .3s; min-height: 44px; }
.tb:hover { background: var(--pr); color: #fff; transform: translateY(-2px); }
.sp { flex: 1; }
.ab { border: none; padding: 10px 20px; border-radius: 12px; font-size: 13px; font-weight: 700; transition: all .3s; box-shadow: var(--sh); min-height: 44px; }
.ab:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(0,0,0,.15); }
.lgd { display: flex; gap: 12px; flex-wrap: wrap; padding: 12px 16px; background: var(--sf); border-radius: 12px; border: 1px solid var(--bd); margin-bottom: 16px; font-size: 13px; box-shadow: var(--sh); }
.li { display: flex; align-items: center; gap: 6px; color: var(--ts); white-space: nowrap; }
.lic { width: 24px; height: 24px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 13px; }
.lsep { width: 1px; background: var(--bd); }
.cg { display: grid; grid-template-columns: repeat(7, 1fr); gap: 8px; margin-bottom: 24px; }
.ch { text-align: center; padding: 14px 0; font-weight: 800; font-size: 14px; color: var(--ts); letter-spacing: 1px; text-transform: uppercase; }
.ch.we { color: var(--dg); }
.cd { background: var(--sf); border: 2px solid var(--bd); border-radius: 16px; padding: 12px; min-height: 140px; transition: all .3s cubic-bezier(.22,1,.36,1); position: relative; overflow: hidden; }
.cd::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(135deg, #667eea, #764ba2); opacity: 0; transition: opacity .3s; }
.cd:hover { box-shadow: 0 12px 40px rgba(0,0,0,.15); transform: translateY(-4px); border-color: var(--pr); }
.cd:hover::before { opacity: 1; }
.cd.today { border: 3px solid var(--pr); background: linear-gradient(135deg, var(--pb), #f0f4ff); box-shadow: 0 0 0 4px rgba(67,97,238,.1); }
.cd.hol { background: linear-gradient(135deg, #fffbf0, #fff5e0); border-color: var(--wn); }
.dn { font-size: 18px; font-weight: 800; color: var(--tx); margin-bottom: 8px; display: flex; align-items: center; gap: 6px; }
.dn.tn { font-weight: 800; color: var(--pr); font-size: 20px; }
.dn .badge { font-size: 11px; color: #d97706; font-weight: 700; margin-bottom: 6px; background: #fff8e1; padding: 3px 8px; border-radius: 6px; display: inline-block; }
.et { display: flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 700; padding: 6px 10px; border-radius: 10px; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; transition: all .2s; }
.et:hover { transform: scale(1.03); }
.et.lv { border: 2px solid; font-size: 13px; padding: 6px 10px; animation: pulse 2s infinite; }
@keyframes pulse { 0%,100% { opacity: 1; } 50% { opacity: .7; } }
.rw { overflow-x: auto; border-radius: var(--rd); border: 1px solid var(--bd); background: var(--sf); box-shadow: var(--sh); }
.rt { width: 100%; border-collapse: collapse; font-size: 13px; }
.rt th { padding: 12px 6px; text-align: center; background: #f8fafc; border-bottom: 2px solid var(--bd); font-weight: 700; min-width: 40px; }
.rt th.sk { position: sticky; left: 0; z-index: 3; min-width: 160px; text-align: left; padding-left: 16px; }
.rt th.tc { background: var(--pb); }
.rt th.hc { background: #fffbeb; color: #d97706; }
.rt th .dl { font-size: 10px; opacity: .7; }
.rt td { text-align: center; padding: 4px; border-bottom: 1px solid #f1f5f9; }
.rt td.sk { position: sticky; left: 0; background: #fff; z-index: 2; text-align: left; padding: 10px 16px; }
.rt td.tc { background: #f0f7ff; }
.ec { display: flex; align-items: center; gap: 10px; }
.eav { font-size: 24px; }
.eimg { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; }
.en { font-weight: 700; font-size: 13px; }
.er { font-size: 11px; color: var(--ts); }
.sc { width: 38px; height: 38px; border-radius: 10px; display: flex; align-items: center; justify-content: center; margin: 0 auto; font-size: 16px; cursor: pointer; transition: transform .2s; }
.sc:hover { transform: scale(1.3); }
.sg { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 16px; margin-bottom: 24px; }
.stc { background: var(--sf); border-radius: 20px; padding: 24px; border: 1px solid var(--bd); box-shadow: var(--sh); transition: all .3s; }
.stc:hover { box-shadow: 0 12px 40px rgba(0,0,0,.15); transform: translateY(-4px); }
.sth { display: flex; align-items: center; gap: 14px; margin-bottom: 18px; }
.sta { font-size: 40px; }
.sti { width: 48px; height: 48px; border-radius: 50%; object-fit: cover; }
.stn { font-size: 18px; font-weight: 700; }
.str { font-size: 12px; color: var(--ts); }
.stl { font-size: 12px; font-weight: 800; color: var(--ts); margin: 14px 0 10px; text-transform: uppercase; letter-spacing: .8px; }
.sts { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 16px; }
.stt { display: flex; align-items: center; gap: 5px; padding: 6px 12px; border-radius: 8px; font-size: 13px; font-weight: 600; }
.qr { margin-bottom: 10px; }
.qh { display: flex; justify-content: space-between; font-size: 12px; color: var(--ts); margin-bottom: 6px; }
.total-bar { background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 14px; padding: 16px 20px; margin-bottom: 16px; color: #fff; box-shadow: 0 4px 16px rgba(102,126,234,.3); }
.total-bar .tbl { font-size: 12px; opacity: .9; font-weight: 600; }
.total-bar .tbv { font-size: 24px; font-weight: 800; }
.total-bar .tbb { height: 10px; background: rgba(255,255,255,.25); border-radius: 5px; margin-top: 8px; overflow: hidden; }
.total-bar .tbf { height: 100%; background: #fff; border-radius: 5px; transition: width .5s ease; }
.ps { margin-top: 24px; }
.pt { font-size: 18px; font-weight: 700; margin-bottom: 14px; }
.pc { background: var(--sf); border-radius: 14px; padding: 16px 20px; border: 1px solid var(--bd); margin-bottom: 10px; display: flex; justify-content: space-between; align-items: center; gap: 14px; flex-wrap: wrap; box-shadow: var(--sh); transition: all .3s; }
.pc:hover { box-shadow: 0 4px 20px rgba(0,0,0,.1); transform: translateX(4px); }
.ba { border: none; background: linear-gradient(135deg, #06d6a0, #00b888); color: #fff; padding: 10px 20px; border-radius: 10px; font-size: 13px; font-weight: 700; min-height: 44px; transition: all .3s; }
.ba:hover { transform: translateY(-2px); box-shadow: 0 4px 16px rgba(6,214,160,.3); }
.br { border: 2px solid var(--dg); background: #fff; color: var(--dg); padding: 10px 20px; border-radius: 10px; font-size: 13px; font-weight: 700; min-height: 44px; transition: all .3s; }
.br:hover { background: var(--dg); color: #fff; transform: translateY(-2px); }
.mo { position: fixed; inset: 0; z-index: 1000; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,.5); backdrop-filter: blur(12px); opacity: 0; pointer-events: none; transition: opacity .3s; }
.mo.show { opacity: 1; pointer-events: auto; }
.md { background: var(--sf); border-radius: 24px; padding: 32px; min-width: 420px; max-width: 800px; width: 92vw; box-shadow: 0 24px 60px rgba(0,0,0,.2); max-height: 88vh; overflow-y: auto; transform: scale(0.9) translateY(20px); transition: all .3s cubic-bezier(.22,1,.36,1); }
.mo.show .md { transform: scale(1) translateY(0); }
.mh { display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px; padding-bottom: 16px; border-bottom: 2px solid var(--bd); }
.mt { font-size: 22px; font-weight: 800; background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; }
.mc { border: none; background: #f1f5f9; width: 38px; height: 38px; border-radius: 12px; font-size: 16px; display: flex; align-items: center; justify-content: center; transition: all .3s; }
.mc:hover { background: var(--db); color: var(--dg); transform: rotate(90deg) scale(1.1); }
.row { padding: 16px; border-radius: 14px; margin-bottom: 10px; border: 2px solid var(--bd); cursor: pointer; transition: all .3s; }
...
.rs { margin-left: auto; padding: 6px 14px; border-radius: 10px; font-size: 12px; font-weight: 700; }
.pg { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 12px; }
.pl { display: inline-flex; align-items: center; gap: 6px; padding: 10px 18px; border-radius: 24px; border: 2px solid transparent; font-size: 13px; font-weight: 600; background: #f8fafc; color: var(--ts); transition: all .3s; cursor: pointer; }
.pl:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(0,0,0,.08); }
.pl.on { border-color: var(--pr); background: var(--pb); color: var(--pr); transform: scale(1.05); box-shadow: 0 4px 14px rgba(67,97,238,.2); }
.sla { font-size: 12px; font-weight: 800; color: var(--ts); margin: 14px 0 10px; text-transform: uppercase; letter-spacing: 1px; }
.btn { width: 100%; padding: 14px 24px; border: none; border-radius: 14px; font-size: 15px; font-weight: 800; color: #fff; background: linear-gradient(135deg, #667eea, #764ba2); margin-top: 18px; transition: all .3s; cursor: pointer; box-shadow: 0 4px 16px rgba(102,126,234,.3); position: relative; overflow: hidden; min-height: 48px; }
...
.btn:hover { transform: translateY(-2px); box-shadow: 0 6px 24px rgba(102,126,234,.4); }
.btn:active { transform: translateY(0); }
.btn:disabled { opacity: .5; cursor: not-allowed; transform: none; }
.fg { margin-bottom: 20px; }
.fl { display: block; font-size: 13px; font-weight: 800; color: var(--ts); margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.8px; }
.fi { width: 100%; padding: 14px 18px; border: 2px solid var(--bd); border-radius: 14px; font-size: 15px; font-family: inherit; outline: none; transition: all .3s; background: var(--sf); color: var(--tx); min-height: 48px; }
.fi:focus { border-color: var(--pr); box-shadow: 0 0 0 4px rgba(67,97,238,.1); transform: translateY(-2px); }
.fi:hover { border-color: var(--pr); }
textarea.fi { resize: vertical; min-height: 100px; }
.tst { position: fixed; top: 24px; right: 24px; z-index: 2000; background: var(--sf); padding: 16px 24px; border-radius: 14px; box-shadow: 0 12px 40px rgba(0,0,0,.2); font-weight: 600; font-size: 14px; border-left: 4px solid var(--su); transform: translateX(120%); transition: transform .3s ease; max-width: 400px; }
.tst.show { transform: translateX(0); animation: slideIn .3s ease; }
...
.save-ok { display: inline-flex; align-items: center; gap: 5px; color: var(--su); font-size: 13px; font-weight: 700; margin-left: 10px; opacity: 0; transition: opacity .3s; }
.save-ok.show { opacity: 1; }
.dp-wrap { position: relative; }
.dp-input { width: 100%; padding: 12px 16px; border: 2px solid var(--bd); border-radius: 12px; font-size: 14px; font-family: inherit; outline: none; cursor: pointer; background: var(--sf); transition: all .2s; min-height: 48px; }
.dp-input:focus { border-color: var(--pr); }
.dp-pop { position: absolute; top: 100%; left: 50%; transform: translateX(-50%); z-index: 1100; background: var(--sf); border-radius: 14px; box-shadow: 0 12px 40px rgba(0,0,0,.15); border: 1px solid var(--bd); padding: 14px; margin-top: 6px; min-width: 280px; max-width: 320px; }
...
.dp-nav { border: none; background: #f1f5f9; width: 32px; height: 32px; border-radius: 8px; font-size: 14px; font-weight: 700; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all .2s; }
.dp-nav:hover { background: #e2e8f0; transform: scale(1.1); }
.dp-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 3px; text-align: center; }
.dp-dow { font-size: 11px; font-weight: 700; color: var(--ts); padding: 6px 0; }
.dp-day { padding: 8px 0; border-radius: 8px; font-size: 13px; cursor: pointer; transition: all .2s; font-weight: 500; }
.dp-day:hover { background: var(--pb); color: var(--pr); }
.dp-day.sel { background: var(--pr); color: #fff; font-weight: 700; }
.dp-day.today { border: 2px solid var(--pr); }
.dp-day.empty { cursor: default; }
.dp-day.empty:hover { background: transparent; }
[data-theme="dark"] { --bg: #0f172a; --sf: #1e293b; --bd: #334155; --tx: #f1f5f9; --ts: #94a3b8; --pr: #6c8dff; --pb: #1e3a8a; --dg: #ff6b8a; --db: #2d1520; --su: #34d399; --sb: #0a2e20; --wn: #fbbf24; --wb: #2d2006; --sh: 0 2px 12px rgba(0,0,0,.4); --sl: 0 12px 40px rgba(0,0,0,.6); }
[data-theme="dark"] body { background: var(--bg); color: var(--tx); }
[data-theme="dark"] .cd { background: var(--sf); border-color: var(--bd); }
[data-theme="dark"] .cd:hover { box-shadow: 0 12px 40px rgba(0,0,0,.5); }
...
[data-theme="dark"] .nb:hover { background: var(--pr); color: #fff; }
...
[data-theme="dark"] .tab.on { background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; }
...
@media (max-width: 768px) {
  .ctn { padding: 16px; padding-bottom: 90px; }
  .hdr { flex-direction: column; text-align: center; padding: 16px; margin-bottom: 16px; }
  .hdr h1 { font-size: 24px; }
  .tabs { display: none; }
  .cg { gap: 4px; }
  .cd { padding: 8px; min-height: 100px; }
  .cd:hover { transform: none; }
  .cd:active { transform: scale(0.98); }
  .dn { font-size: 14px; margin-bottom: 4px; }
  .et { font-size: 11px; padding: 4px 8px; margin-bottom: 2px; }
  .md { min-width: 0; width: 95vw; padding: 20px; margin: 10px; border-radius: 20px 20px 0 0; transform: translateY(100%); }
  .mo.show .md { transform: translateY(0); }
  .md::before { content: ''; position: absolute; top: 12px; left: 50%; transform: translateX(-50%); width: 40px; height: 4px; background: var(--bd); border-radius: 20px; }
  .sg { grid-template-columns: 1fr; }
  .mnv { padding: 12px 14px; flex-wrap: wrap; justify-content: center; }
  .mnv h2 { font-size: 18px; width: 100%; order: 1; margin-bottom: 10px; }
  .nb { width: 38px; height: 38px; font-size: 16px; }
  .tb { padding: 8px 16px; font-size: 12px; }
  .btn, .ab, .tab, .nb, .tb, .ba, .br, .fi { min-height: 44px; }
  .tst { left: 12px; right: 12px; top: 12px; max-width: none; }
  .pc { flex-direction: column; align-items: flex-start; padding: 14px 16px; }
  .ba, .br { width: 100%; }
  .rt { font-size: 11px; }
  .rt th, .rt td { padding: 8px 3px; min-width: 36px; }
  .rt th.sk, .rt td.sk { min-width: 100px; font-size: 11px; }
  .lgd { padding: 10px 14px; font-size: 11px; overflow-x: auto; scrollbar-width: none; }
  .lgd::-webkit-scrollbar { display: none; }
  .ub { flex-wrap: wrap; justify-content: center; gap: 10px; }
}
@media (max-width: 480px) {
  .cd { min-height: 80px; padding: 6px; }
  .dn { font-size: 12px; }
  .et { font-size: 10px; }
}
.bottom-nav { position: fixed; bottom: 0; left: 0; right: 0; background: var(--sf); border-top: 1px solid var(--bd); padding: 8px 0; display: none; z-index: 100; box-shadow: 0 -4px 20px rgba(0,0,0,.1); }
.bottom-nav-items { display: flex; justify-content: space-around; align-items: center; max-width: 600px; margin: 0 auto; }
.bottom-nav-item { display: flex; flex-direction: column; align-items: center; gap: 4px; padding: 8px 12px; border: none; background: transparent; color: var(--ts); font-size: 11px; font-weight: 600; cursor: pointer; transition: all .3s; border-radius: 12px; flex: 1; min-height: 56px; }
.bottom-nav-item span { font-size: 20px; }
.bottom-nav-item:active { transform: scale(0.95); }
.bottom-nav-item.active { color: var(--pr); background: var(--pb); }
@media (max-width: 768px) { .bottom-nav { display: block; } }
@supports (padding: max(0px)) { .bottom-nav { padding-bottom: max(8px, env(safe-area-inset-bottom)); } }
.scroll-to-top { position: fixed; bottom: 90px; right: 24px; width: 50px; height: 50px; border-radius: 50%; background: linear-gradient(135deg, #667eea, #764ba2); color: white; border: none; font-size: 24px; font-weight: bold; cursor: pointer; box-shadow: 0 4px 20px rgba(102,126,234,.4); opacity: 0; pointer-events: none; transition: all .3s; z-index: 99; }
.scroll-to-top.show { opacity: 1; pointer-events: auto; }
.scroll-to-top:hover { transform: scale(1.1) translateY(-4px); box-shadow: 0 6px 28px rgba(102,126,234,.5); }
html { scroll-behavior: smooth; }
::-webkit-scrollbar { width: 8px; height: 8px; }
::-webkit-scrollbar-track { background: var(--bg); border-radius: 4px; }
::-webkit-scrollbar-thumb { background: var(--ts); border-radius: 4px; transition: background .3s; }
::-webkit-scrollbar-thumb:hover { background: var(--pr); }
::selection { background: var(--pb); color: var(--pr); }
:focus-visible { outline: 2px solid var(--pr); outline-offset: 2px; }
@keyframes shimmer { 0% { background-position: -1000px 0; } 100% { background-position: 1000px 0; } }
.loading { background: linear-gradient(90deg, var(--bd) 0%, var(--sf) 50%, var(--bd) 100%); background-size: 1000px 100%; animation: shimmer 2s infinite; border-radius: 12px; }
@media (prefers-reduced-motion: reduce) { *, *::before, *::after { animation-duration: 0.01ms !important; animation-iteration-count: 1 !important; transition-duration: 0.01ms !important; } }
@media print { .bottom-nav, .scroll-to-top, .tst, .tabs, .ubtn { display: none !important; } * { box-shadow: none !important; } }
</head><body>
...
function rKpi() {
  ...
  subTabs.forEach(([k, l]) => tb.appendChild(h('button', { style: { padding: '8px 16px', borderRadius: '10px', fontSize: '13px', fontWeight: 700, border: '2px solid', borderColor: D.kpiTab === k ? '#818cf8' : 'rgba(255,255,255,0.1)', background: D.kpiTab === k ? 'rgba(99,102,241,0.15)' : 'rgba(255,255,255,0.03)', color: D.kpiTab === k ? '#818cf8' : '#94a3b8', cursor: 'pointer', transition: 'all .15s' }, onClick: () => { D.kpiTab = k; render(); } }, l)));
  ...
  if (D.kpiTab === 'summary') {
    // === HERO BANNER ===
    const hero = h('div', { style: { background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 40%, #312e81 100%)', borderRadius: '20px', padding: '28px', color: '#fff', marginBottom: '20px', position: 'relative', overflow: 'hidden' } });
    // Decorative elements
    hero.appendChild(h('div', { style: { position: 'absolute', top: '-40px', right: '-40px', width: '200px', height: '200px', background: 'radial-gradient(circle, rgba(239,68,68,0.15) 0%, transparent 70%)', borderRadius: '50%' } }));
    hero.appendChild(h('div', { style: { position: 'absolute', bottom: '-30px', left: '20%', width: '160px', height: '160px', background: 'radial-gradient(circle, rgba(99,102,241,0.1) 0%, transparent 70%)', borderRadius: '50%' } }));
    hero.appendChild(h('div', { style: { fontSize: '13px', fontWeight: 600, opacity: 0.5, marginBottom: '20px', textTransform: 'uppercase', letterSpacing: '2px', position: 'relative', zIndex: 1 } }, '⚡ KPI DASHBOARD ' + (D.y+543)));

    const heroGrid = h('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', position: 'relative', zIndex: 1 } });
    const heroItems = [
      { val: sum.totals.count, icon: '⚠️', label: 'ข้อผิดพลาดรวม', color: '#f87171', bg: 'rgba(239,68,68,0.12)' },
      { val: sum.totals.points, icon: '🔢', label: 'แต้มรวม', color: '#fbbf24', bg: 'rgba(251,191,36,0.12)' },
      { val: (sum.totals.damage || 0).toLocaleString() + ' ฿', icon: '💸', label: 'ค่าเสียหาย', color: '#f97316', bg: 'rgba(249,115,22,0.12)' },
    ];
    heroItems.forEach(item => {
      const card = h('div', { style: { background: item.bg, borderRadius: '14px', padding: '18px', textAlign: 'center', border: '1px solid ' + item.color + '20', backdropFilter: 'blur(8px)' } });
      card.appendChild(h('div', { style: { fontSize: '28px', marginBottom: '6px' } }, item.icon));
      const numEl = h('div', { style: { fontSize: '32px', fontWeight: 900, color: item.color, letterSpacing: '-1px' } }, '0');
      card.appendChild(numEl);
      card.appendChild(h('div', { style: { fontSize: '11px', opacity: 0.6, marginTop: '2px' } }, item.label));
      heroGrid.appendChild(card);
      // Animate number
      setTimeout(() => {
        const target = typeof item.val === 'string' ? item.val : item.val;
        if (typeof item.val === 'number' && item.val > 0) {
          let cur = 0;
          const step = () => { cur += Math.ceil(item.val / 15); if (cur >= item.val) { numEl.textContent = String(item.val); return; } numEl.textContent = String(cur); requestAnimationFrame(step); };
          requestAnimationFrame(step);
    ...
    if (sum.totals.count === 0) {
      hero.appendChild(h('div', { style: { textAlign: 'center', marginTop: '20px', padding: '14px', background: 'rgba(34,197,94,0.12)', borderRadius: '12px', border: '1px solid rgba(34,197,94,0.2)', position: 'relative', zIndex: 1 } },
        h('div', { style: { fontSize: '32px', marginBottom: '4px' } }, '🎉'),
        h('div', { style: { fontSize: '16px', fontWeight: 800, color: '#34d399' } }, 'ไม่มีข้อผิดพลาดเลย! เยี่ยมมาก!')));
    }
    w.appendChild(hero);
    ...
    sortedEmps.forEach((emp, idx) => {
      ...
      // Stats row
      const statsRow = h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' } });
      const ptsBg = ok ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)';
      const ptsColor = ok ? '#22c55e' : '#ef4444';
      statsRow.appendChild(h('div', { style: { textAlign: 'center', padding: '10px', background: ptsBg, borderRadius: '10px' } },
        h('div', { style: { fontSize: '22px', fontWeight: 800, color: ptsColor } }, ok ? '✨' : String(d.total_points)),
        h('div', { style: { fontSize: '10px', color: '#94a3b8' } }, ok ? '0 แต้ม' : 'แต้ม')));
      statsRow.appendChild(h('div', { style: { textAlign: 'center', padding: '10px', background: 'rgba(99,102,241,0.1)', borderRadius: '10px' } },
        h('div', { style: { fontSize: '22px', fontWeight: 800, color: '#818cf8' } }, String(d.error_count)),
        h('div', { style: { fontSize: '10px', color: '#94a3b8' } }, 'ครั้ง')));
    ...
    if (!my.length) { w.appendChild(h('div', { style: { textAlign: 'center', padding: '40px', background: 'rgba(34,197,94,0.06)', borderRadius: '16px', border: '1px solid rgba(34,197,94,0.15)' } }, h('div', { style: { fontSize: '48px', marginBottom: '8px' } }, '🏆'), h('div', { style: { fontSize: '16px', fontWeight: 700, color: '#34d399' } }, 'ยังไม่มีข้อผิดพลาด! 🎉'))); return w; }
    ...
    w.appendChild(h('div', { style: { display: 'flex', gap: '12px', marginBottom: '16px' } },
      h('div', { style: { flex: 1, background: 'rgba(239,68,68,0.08)', borderRadius: '12px', padding: '16px', textAlign: 'center', border: '1px solid rgba(239,68,68,0.15)' } }, h('div', { style: { fontSize: '12px', color: '#f87171' } }, 'แต้มรวม'), h('div', { style: { fontSize: '28px', fontWeight: 800, color: '#f87171' } }, String(mp))),
      h('div', { style: { flex: 1, background: 'rgba(249,115,22,0.08)', borderRadius: '12px', padding: '16px', textAlign: 'center', border: '1px solid rgba(249,115,22,0.15)' } }, h('div', { style: { fontSize: '12px', color: '#fb923c' } }, 'ค่าเสียหาย'), h('div', { style: { fontSize: '28px', fontWeight: 800, color: '#fb923c' } }, md.toLocaleString() + ' ฿'))));
    my.forEach(er => {
      w.appendChild(h('div', { style: { padding: '12px 14px', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', marginBottom: '6px', border: '1px solid rgba(255,255,255,0.06)', borderLeft: '4px solid ' + (er.cat_color || '#6366f1'), fontSize: '13px' } },
        h('div', { style: { display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' } },
          ...
          h('b', {}, er.detail_desc || er.note || '—'), h('span', { style: { color: '#f87171', fontWeight: 700 } }, '🔢 ' + er.points)),
        h('div', { style: { fontSize: '11px', color: '#94a3b8', marginTop: '4px' } }, '📅 ' + fmtDate(er.date) + (er.damage_cost > 0 ? ' | 💸 ' + er.damage_cost.toLocaleString() + ' ฿' : ''))));
    });
      ...
      items.forEach(reward => {
        ...
        if (isOnSale) {
          rCard.appendChild(h('div', { style: { marginBottom: '8px' } },
            ...
            h('span', { style: { fontSize: '9px', padding: '2px 4px', borderRadius: '4px', background: '#dc2626', color: '#fff', fontWeight: 700, marginLeft: '4px' } }, '-' + flashDisc + '%')));
        } else {
        ...
        if (reward.type === 'cash') rCard.appendChild(h('div', { style: { fontSize: '10px', color: '#64748b', marginBottom: '6px' } }, '= ' + (saleCost * rate) + ' บาท'));
...
function rRewardMgr() {
  ...
  // === FLASH SALE SECTION ===
  let flashSale = null;
  try { flashSale = JSON.parse(D.set.flash_sale || 'null'); } catch(e) {}
  const isFlashActive = flashSale && flashSale.active && flashSale.expires && new Date(flashSale.expires) > new Date();

  const flashCard = h('div', { style: { background: isFlashActive ? 'linear-gradient(135deg, #dc2626, #f59e0b)' : 'linear-gradient(135deg, #1e293b, #334155)', borderRadius: '14px', padding: '16px', marginBottom: '16px', color: '#fff' } });
  const flashHdr = h('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' } });
  flashHdr.appendChild(h('div', { style: { display: 'flex', alignItems: 'center', gap: '8px' } },
    h('span', { style: { fontSize: '20px' } }, '⚡'),
    h('div', {},
  ...
  if (isFlashActive) {
    flashHdr.appendChild(h('button', { style: { background: 'rgba(0,0,0,0.3)', border: 'none', color: '#fff', padding: '6px 14px', borderRadius: '8px', fontWeight: 700, fontSize: '11px', cursor: 'pointer' }, onClick: async () => {
      await api('/api/settings', 'PUT', { key: 'flash_sale', value: JSON.stringify({ active: false }) });
      D.set.flash_sale = JSON.stringify({ active: false });
  ...
  // === ADD NEW REWARD ===
  const addCard = h('div', { style: { background: '#f0fdf4', borderRadius: '14px', padding: '16px', marginBottom: '16px', border: '1px solid #bbf7d0' } });
  addCard.appendChild(h('div', { style: { fontWeight: 700, fontSize: '13px', color: '#16a34a', marginBottom: '10px' } }, '➕ เพิ่มรางวัลใหม่'));
  const addR1 = h('div', { style: { display: 'grid', gridTemplateColumns: '50px 1fr 70px', gap: '8px', marginBottom: '8px' } });
  addR1.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'ไอคอน'), h('input', { type: 'text', id: 'rw-icon', className: 'fi', value: '🎁', style: { fontSize: '18px', textAlign: 'center' } })));
  addR1.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'ชื่อรางวัล'), h('input', { type: 'text', id: 'rw-name', className: 'fi', placeholder: 'เช่น กาแฟ 1 แก้ว' })));
  addR1.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'แต้ม'), h('input', { type: 'number', id: 'rw-cost', className: 'fi', value: '50' })));
  addCard.appendChild(addR1);
  const addR2 = h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 60px auto', gap: '8px', alignItems: 'end' } });
  addR2.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'หมวด'),
  ...
  addR2.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'ประเภท'),
  ...
  addR2.appendChild(h('div', {}, h('label', { style: { fontSize: '10px', color: '#64748b' } }, 'สต๊อก'), h('input', { type: 'number', id: 'rw-stock', className: 'fi', value: '-1', title: '-1=ไม่จำกัด' })));
  addR2.appendChild(h('button', { style: { padding: '8px 16px', background: '#16a34a', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', fontSize: '13px' }, onClick: async () => {
    ...
    try {
      ...
      toast('✅ เพิ่มรางวัลสำเร็จ'); D.walletLoaded = false; closeModal(); render();
    } catch (er) { toast(er.message, true); }
  } }, '+ เพิ่ม'));
  addCard.appendChild(addR2);
  m.appendChild(addCard);

  // === EXISTING REWARDS ===
  const grouped = {};
  (D.rewardsList || []).forEach(rw => { const c = rw.category || 'item'; if (!grouped[c]) grouped[c] = []; grouped[c].push(rw); });
  const totalRewards = (D.rewardsList || []).length;
  m.appendChild(h('div', { style: { fontWeight: 700, fontSize: '14px', marginBottom: '8px', color: '#1e293b' } }, '📦 รางวัลทั้งหมด (' + totalRewards + ')'));

  Object.entries(REWARD_CATS).forEach(([catKey, catLabel]) => {
    const items = grouped[catKey];
    if (!items || items.length === 0) return;
    m.appendChild(h('div', { style: { fontSize: '11px', fontWeight: 700, color: '#94a3b8', marginTop: '10px', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.5px' } }, catLabel));
    items.forEach(rw => {
      const stockTxt = rw.stock === -1 || rw.stock === null ? '♾️' : rw.stock === 0 ? '❌' : '📦' + rw.stock;
      const row = h('div', { style: { display: 'flex', alignItems: 'center', padding: '10px 14px', background: '#fff', borderRadius: '12px', marginBottom: '6px', border: '1px solid #e2e8f0', gap: '10px', transition: 'all .15s' } });
      row.onmouseenter = () => { row.style.borderColor = '#818cf8'; row.style.boxShadow = '0 2px 8px rgba(99,102,241,0.1)'; };
      row.onmouseleave = () => { row.style.borderColor = '#e2e8f0'; row.style.boxShadow = 'none'; };
      row.appendChild(h('span', { style: { fontSize: '28px', width: '36px', textAlign: 'center', flexShrink: 0 } }, rw.icon));
      const info = h('div', { style: { flex: 1, minWidth: 0 } });
      info.appendChild(h('div', { style: { fontWeight: 700, fontSize: '13px', color: '#1e293b' } }, rw.name));
      info.appendChild(h('div', { style: { display: 'flex', gap: '8px', fontSize: '11px', color: '#64748b', marginTop: '2px' } },
        h('span', {}, '💰 ' + rw.cost),
        h('span', {}, rw.type === 'cash' ? '💸 เงิน' : '🎁 ของ'),
        h('span', {}, stockTxt)));
      row.appendChild(info);
      // Action buttons
      const acts = h('div', { style: { display: 'flex', gap: '4px', flexShrink: 0 } });
      acts.appendChild(h('button', { style: { width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #e2e8f0', background: '#f8fafc', color: '#3b82f6', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }, onClick: () => {
        // Edit inline
        row.innerHTML = '';
        ...
        row.style.border = '2px solid #818cf8';
        const eIcon = h('input', { type: 'text', className: 'fi', value: rw.icon, style: { fontSize: '18px', textAlign: 'center', padding: '4px', gridRow: '1/3' } });
        const eFields = h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 60px', gap: '4px' } });
        ...
        eFields.appendChild(eName); eFields.appendChild(eCost);
        const eFields2 = h('div', { style: { display: 'grid', gridTemplateColumns: '1fr 1fr 50px auto', gap: '4px', alignItems: 'center' } });
        const eCat = h('select', { className: 'fi', style: { padding: '3px', fontSize: '11px' } });
        ...
        const eType = h('select', { className: 'fi', style: { padding: '3px', fontSize: '11px' } });
        [['item','🎁 ของ'],['cash','💸 เงิน']].forEach(([v,l]) => { const opt = h('option', { value: v }, l); if (rw.type===v) opt.selected=true; eType.appendChild(opt); });
        const eStock = h('input', { type: 'number', className: 'fi', value: String(rw.stock??-1), style: { padding: '3px', fontSize: '11px' } });
        const eSave = h('div', { style: { display: 'flex', gap: '4px' } });
        eSave.appendChild(h('button', { style: { background: '#16a34a', border: 'none', color: '#fff', padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }, onClick: async () => {
          try { await api('/api/rewards/' + rw.id, 'PUT', { name: eName.value.trim(), icon: eIcon.value.trim(), cost: +eCost.value, type: eType.value, stock: +eStock.value, category: eCat.value }); toast('✅ แก้ไขสำเร็จ'); D.walletLoaded = false; closeModal(); render(); } catch(er) { toast(er.message, true); }
        } }, '💾'));
        eSave.appendChild(h('button', { style: { background: '#e2e8f0', border: 'none', color: '#64748b', padding: '4px 8px', borderRadius: '6px', fontSize: '12px', fontWeight: 700, cursor: 'pointer' }, onClick: () => { closeModal(); render(); setTimeout(() => openModal('rewardMgr'), 50); } }, '✕'));
        eFields2.appendChild(eCat); eFields2.appendChild(eType); eFields2.appendChild(eStock); eFields2.appendChild(eSave);
        row.appendChild(eIcon); const rightCol = h('div', {}); rightCol.appendChild(eFields); rightCol.appendChild(eFields2); row.appendChild(rightCol);
      } }, '✏️'));
      acts.appendChild(h('button', { style: { width: '32px', height: '32px', borderRadius: '8px', border: '1px solid #fee2e2', background: '#fef2f2', color: '#dc2626', fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }, onClick: async () => {
        if (!confirm('ลบ ' + rw.icon + ' ' + rw.name + '?')) return;
        await api('/api/rewards/' + rw.id, 'DELETE'); toast('ลบแล้ว'); D.walletLoaded = false; closeModal(); render();
      } }, '🗑️'));
  ...
  if (totalRewards === 0) m.appendChild(h('div', { style: { textAlign: 'center', padding: '30px', color: '#94a3b8', fontSize: '13px' } }, 'ยังไม่มีรางวัล — เพิ่มด้านบนเลย!'));
...
<nav class="bottom-nav">
  <div class="bottom-nav-items">
    <button class="bottom-nav-item" onclick="D.v='calendar';render()"><span>📅</span>ปฏิทิน</button>
    <button class="bottom-nav-item" onclick="D.v='roster';render()"><span>📋</span>ตาราง</button>
    <button class="bottom-nav-item" onclick="D.v='stats';render()"><span>📊</span>สถิติ</button>
    <button class="bottom-nav-item" onclick="D.v='pending';render()"><span>🔔</span>อนุมัติ</button>
  </div>
</nav>
